...
from allennlp.training.callbacks import Callback, CallbackHandler, EventType

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name


@TrainerBase.register("callback")
class TrainerWithCallbacks(TrainerBase):
    def __init__(self,
                 model: Model,
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_group in train_generator_tqdm:
            ...
            self.callbacks.on_event(EventType.STEP_END)
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationError("Could not recover training from the checkpoint.  Did you mean to output to "
                                     ...
                                     "directory?")
        self.callbacks.on_event(EventType.TRAIN_BEGIN)
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            self.callbacks.on_event(EventType.EPOCH_BEGIN)

            epoch_start_time = time.time()
            train_metrics = self._train_epoch(epoch)

            self.callbacks.on_event(EventType.EPOCH_END)
        ...
        self.callbacks.on_event(EventType.TRAIN_END)
        return metrics
